(function() {
  var CSS_MODULE_HELPER, EOL, EXPORTS_REGEXP, PluginError, Q, Vinyl, _, beautify, chalk, compile, compileCss, compileLess, compileSass, crypto, cssBase64img, cssSprite, fixDefineParams, fs, getBodyDeps, getErrorStack, getUnixStylePath, gulpCssSprite, htmlBase64img, less, marked, minifier, path, sass, sus, through2, uglify;
  ...
  compileLess = function(file, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      lessStream.pipe(through2.obj(function(file, enc, next) {
        return Q.Promise(function(resolve, reject) {
          if (opt.postcss) {
  ...
  module.exports = function(opt = {}) {
    return through2.obj(function(file, enc, next) {
      ...
      return module.exports.compile(file, opt).then((file) => {
        this.push(file);
        return next();
      }, (err) => {
        return this.emit('error', new PluginError('gulp-mt2amd', err));
      }).done();
    });
  };
      ...
      if (extName === '.json') {
        ...
        return compile(file, opt).then(function(processed) {
          ...
          content = [
            ...
            "  _$out_ += '" + processed.contents.toString().replace(/<\/script>/ig,
            '</s<%=""%>cript>').replace(/\r\n|\n|\r/g,
            "\v").replace(/(?:^|%>).*?(?:<%|$)/g,
            function($0) {
              var res;
            ...
            "\\$1").replace(/\t/g,
            "").replace(/>\v +/g,
            ">" + replacedSpace).replace(/\v +</g,
            replacedSpace + "<").replace(/\v+/g,
            replacedSpace).replace(/ +/g,
            " ");
              if (!(/(?:^|%>) *(?:<%|$)/.test(res))) {
                res = res.replace(/(^|%>) /,
            "$1");
            ...
            EOL).replace(/<%==([\s\S]*?)%>/mg,
            "' + $encodeHtml($1) + '").replace(/<%=([\s\S]*?)%>/mg,
            "' + ($1) + '").replace(/<%(<-)?/g,
            "';" + EOL + "  ").replace(/->(\w+)%>/g,
            EOL + "  $1 += '").split("%>").join(EOL + "  _$out_ += '") + "';",
            "  return _$out_;",
            "}"
